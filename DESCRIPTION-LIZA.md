...
- **Préparer l'environnement d'exécution des agents** :
  - activer le projet — `.claude/hooks/` + `.claudeignore` (posés par `liza init`, à vérifier) ;
  - si la toolchain cible n'est pas un écosystème pré-configuré (voir [Support multi-langage cible](#support-multi-langage-cible)), ajouter ses commandes à l'allowlist `.claude/settings.json` (ex. `Bash(mvn:*)`, `Bash(java:*)`, `Bash(./mvnw:*)`) — une commande hors liste échoue **silencieusement** pour un agent headless ;
  - s'assurer que cette toolchain (JDK, Maven, …) est réellement **installée** dans l'environnement d'exécution des agents / le container Docker — l'allowlist autorise, elle n'installe pas ;
  - revoir `AGENT_TOOLS.md` selon l'outillage réellement présent (procédure : `support-docs/CUSTOMIZING_AGENT_TOOLS.md`).
...
**Contrepoint — le mécanisme `lessons/agents/`.** Les fiches `lessons/agents/` capturent les frictions opérationnelles rencontrées en run (paths de worktrees mal construits, lecture de gros fichiers de tests, symlinks détruits par l'outil Edit, prérequis de build manquants) — des frictions, pas des échecs de qualité du code produit, et elles s'accumulent pour les runs suivants. Souvent l'agent les absorbe seul : sur un sprint Java, un coder a rencontré une incompatibilité **tierce** (google-java-format 1.22 × JDK 25 — `NoSuchMethodError` sur le hook `pretty-format-java`, pas un défaut de Liza), l'a corrigée (épinglage d'une version compatible via un flag du hook, sans downgrade du JDK), et a écrit la fiche correspondante dans le même passage, sans intervention humaine.
